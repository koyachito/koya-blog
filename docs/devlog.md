...
### 実施したこと
- Vercel Blobを利用した画像アップロード機能を追加
- 新規投稿・編集画面から本文へ画像Markdownを挿入できるようにした
- 画像形式とファイルサイズのバリデーションを追加
- 本番環境でアップロードと画像表示を確認

### 詰まったところ
- BlobストアがVercelプロジェクトに接続されておらず、ローカル・本番ともにアップロードが失敗した
- Blobストアを作成・接続し、再デプロイすることで解決した

### 学び
- SDKの導入だけではストレージは利用できず、Vercel側でBlobストアの作成とプロジェクトへの接続が必要
- `console.error`で元の例外を残すと、認証情報不足などの原因を特定しやすい